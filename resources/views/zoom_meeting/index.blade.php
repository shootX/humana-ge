...
@section('action-button')
    @auth('web')
        @if (isset($currentWorkspace) && $currentWorkspace->creater->id == Auth::id())
            <a href="{{ route('zoommeeting.Calender', $currentWorkspace->slug) }}" class="btn btn-sm btn-primary"
    ...
    @auth('client')
        <a href="{{ route('zoommeetings.Calender', $currentWorkspace->slug) }}" data-toggle="tooltip"
            title="{{ __('calendar') }}" class="btn btn-sm btn-primary" id=""> <i class="ti ti-calendar"></i>
        </a>
    @endauth
@endsection
                                        ...
                                        <td>
                                            @if ($item->checkDateTime())
                                                @if ($item->status == 'waiting')
                                                    <span
                                                        class="badge bg-info p-2 px-3">{{ ucfirst($item->status) }}</span>
                                                @else
                                                    <span
                                                        class="badge bg-success p-2 px-3">{{ ucfirst($item->status) }}</span>
                                                @endif
                                            @else
