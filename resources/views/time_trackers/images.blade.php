...
<div class="modal-header pb-2 pt-2">
    <h5 class="modal-title" id="exampleModalLongTitle">{{ $tracker->project_task }}
        <small>( {{ $tracker->total }} {{ date('H:i:s, d M ', strtotime($tracker->start_time)) }} )</small>
    </h5>
                        ...
                        @foreach ($images as $image)
                            <div class="swiper-slide" id="slide-{{ $image->id }}">
                                ...
                                <div class="time_in_slider"> {{ date('H:i:s, d M ', strtotime($image->time)) }}
                                    <a href="#"
                                        class=" btn-danger btn btn-sm bs-pass-fn-call"
                                        data-confirm="{{ __('Are You Sure?') }}"
                                        data-text="{{ __('This action can not be undone. Do you want to continue?') }}"
                                        data-id="{{ $image->id }}" title="{{ __('Delete') }}"
                                        data-bs-toggle="tooltip" data-bs-placement="top">
                                        <span class="text-danger"><i class="ti ti-trash text-white"></i></span>
                                    </a>
                                </div>
                            </div>
                        @endforeach
